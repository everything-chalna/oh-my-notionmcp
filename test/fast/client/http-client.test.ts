...
describe('HttpClient', () => {
  ...
  it('normalizes pre-encoded path parameters to avoid double encoding', async () => {
    const mockAxiosInstance = {
      retrievePageProperty: vi.fn().mockResolvedValue({
        data: { object: 'property_item' },
        status: 200,
        headers: { 'content-type': 'application/json' },
      }),
    }

    const encodedPathSpec: OpenAPIV3.Document = {
      openapi: '3.0.0',
    ...
    mockInit.mockResolvedValue(mockAxiosInstance)
    const client = new HttpClient({ baseUrl: 'http://test-api.com' }, encodedPathSpec)
    const operation = encodedPathSpec.paths['/v1/pages/{page_id}/properties/{property_id}']?.get
    if (!operation) {
      throw new Error('Operation not found in encodedPathSpec')
    }
    ...
    expect(mockAxiosInstance.retrievePageProperty).toHaveBeenCalledWith(
      {
        page_id: '312c9946-d64a-80cc-a398-fda09577bfc4',
        property_id: 'b>iH',
      },
  ...
  it('keeps malformed percent-encoded path parameters unchanged', async () => {
    const mockAxiosInstance = {
      retrievePageProperty: vi.fn().mockResolvedValue({
        data: { object: 'property_item' },
        status: 200,
