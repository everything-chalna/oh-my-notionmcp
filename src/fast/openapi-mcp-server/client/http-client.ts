...
export class HttpClient {
  ...
  private normalizePathParameterValue(value: unknown): unknown {
        ...
        if ('name' in param && param.name && param.in) {
          if (param.in === 'path' || param.in === 'query') {
            if (params[param.name] !== undefined) {
              urlParameters[param.name] =
                param.in === 'path'
                  ? this.normalizePathParameterValue(params[param.name])
                  : params[param.name]
              if (!formData) {
